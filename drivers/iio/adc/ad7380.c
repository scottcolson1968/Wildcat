...
#define AD7380_MAX_RATE 4000000

/*
 * AD738x support several SDO lines to increase throughput, but driver currently
 * supports only 1 SDO line (standard SPI transaction)
 */
#define AD7380_NUM_SDO_LINES		2 // 00:2-line 01:1-line 10:4-line 11:1-line
...
#define _AD7380_CHANNEL(index, bits, diff, sign, gain) {			\
	...
	.info_mask_separate = ((gain) ? BIT(IIO_CHAN_INFO_SCALE) : 0) |			\
...
DEFINE_AD7380_8_CHANNEL(ad7383_4_channels, 16, 0, s);
...
static void ad7380_get_sampling_freq(const struct ad7380_state *st, unsigned int *freq)
{
	struct pwm_state conversion_state;

	pwm_get_state(st->conv_trigger, &conversion_state);
	*freq = DIV_ROUND_CLOSEST_ULL(NANO, conversion_state.period-1);
}

static int __ad7380_set_sampling_freq(const struct ad7380_state *st, unsigned int freq)
{
	...
	conv_state.period = DIV_ROUND_CLOSEST(NSEC_PER_SEC, freq)+1;

	printk(KERN_INFO "ad7380 pwm period %d duty cycle %d",conv_state.period,conv_state.duty_cycle);
	return pwm_apply_state(st->conv_trigger, &conv_state);
	
}

static int ad7380_set_sampling_freq(struct iio_dev *indio_dev, unsigned int freq)
{
	const struct ad7380_state *st = iio_priv(indio_dev);
	int ret;
	...
	ret = __ad7380_set_sampling_freq(st, freq);
...
static int ad7380_pwm_get(struct ad7380_state *st)
{
	...
	st->max_rate=AD7380_MAX_RATE;
	return __ad7380_set_sampling_freq(st, st->max_rate);
}
...
{
	printk(KERN_INFO "ad7380_set_ch only used for muxed channels");
	struct spi_transfer xfer = {
		.delay = {
			.value = T_CONVERT_NS,
			.unit = SPI_DELAY_UNIT_NSECS,
...
{
//	struct spi_transfer xfer = st->offload_xfer;//st->seq ? st->seq_xfer : st->normal_xfer;
	...
	if (st->seq) {
		...
		st->normal_xfer.delay.value = t_convert;
		st->normal_xfer.delay.unit = SPI_DELAY_UNIT_NSECS;
		st->normal_xfer.speed_hz = AD7380_REG_WR_SPEED_HZ;
		st->normal_xfer.bits_per_word = scan_type->realbits;
		st->normal_xfer.len = 16;/*BITS_TO_BYTES(scan_type->storagebits) *
			st->chip_info->num_simult_channels;*/
		st->normal_xfer.cs_change;
		xfer[1].rx_nbits = SPI_NBITS_QUAD;
		printk(KERN_INFO "xfer len=%d bits=%d",xfer[1].len,xfer[1].bits_per_word);
//	}
#endif	
	/*
...
//	st->offload_xfer.speed_hz = AD4630_SPI_SAMPLING_SPEED;
	...
	st->offload_xfer.rx_nbits = SPI_NBITS_QUAD;
	st->offload_xfer.bits_per_word = 16;//st->bits_per_word;

	st->offload_xfer.len = 8;/*BITS_TO_BYTES(scan_type->storagebits) *
			st->chip_info->num_simult_channels;*/
	st->offload_xfer.delay.value = 0;//t_convert;
	st->offload_xfer.delay.unit = SPI_DELAY_UNIT_NSECS;
	st->offload_xfer.cs_change_delay.value = 0;//>chip_info->timing_specs->t_csh_ns;
	st->offload_xfer.cs_change_delay.unit = SPI_DELAY_UNIT_NSECS;
	printk(KERN_INFO "xfer len=%d bits=%d",st->offload_xfer.len,st->offload_xfer.bits_per_word);
	spi_message_init_with_transfers(&st->offload_msg, &st->offload_xfer, 1);
}


static int ad7380_sampling_enable(const struct ad7380_state *st, bool enable)
{
...
*/
	...
	ad7380_sampling_enable(st, true);
...
static int ad7380_buffer_postdisable(struct iio_dev *indio_dev)
{
	...
	ret = ad7380_sampling_enable(st, false);
	if (ret)
		goto out_error;
			      ...
			      const struct iio_scan_type *scan_type, int *val)
{
	...
	if (ret < 0)
		return ret;
	for (i=0;i<8;i++)
		printk(KERN_INFO "scan_data[%d]0x%x ",i,st->scan_data[i]);
			   ...
			   int *val, int *val2, long info)
{
	...
	switch (info) {
		...
		ad7380_get_sampling_freq(st, val);
		return IIO_VAL_INT;
			    ...
			    int val2, long mask)
{
	...
	switch (mask) {
	case IIO_CHAN_INFO_SAMP_FREQ:
		return ad7380_set_sampling_freq(indio_dev, val);
		unreachable();
	case IIO_CHAN_INFO_OVERSAMPLING_RATIO:
...
static int ad7380_probe(struct spi_device *spi)
{
	...
	indio_dev->setup_ops = &ad7380_buffer_setup_ops;
